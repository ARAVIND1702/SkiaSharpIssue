...
namespace SkiaSharpIssue.ViewModels
{
 ...
 // if i using the below line instead of line-52 it is working perfectly.
 // using (Stream stream = GetType().Assembly.GetManifestResourceStream("SkiaSharpIssue.Resources.working.svg")) 
            using (Stream stream = monitoringGraphicsSVG.GetStream(svgFrameIndex: svgFrameIndex))
            {
